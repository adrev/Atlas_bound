...
export const RULES_MATRIX: RulesMatrixEntry[] = [
  ...
  },
  {
    ...
    paths: [
      ...
      'server/src/services/chatCommands/spellHandlers.ts',
    ...
    notes: [
      ...
      'Legacy save-or-suck helpers such as Hold Person, Fear, Slow, Bane, and Faerie Fire now roll initial saves through the shared save helper before applying conditions.',
  ...
  },
  {
    ...
    priority: 'P2',
    paths: [
      'client/src/utils/rest.ts',
      'server/src/services/chatCommands/restHandlers.ts',
      'server/src/services/chatCommands/monkHandler.ts',
    ...
    notes: [
      ...
      'Monk Ki and Sorcery Points are persisted as version-guarded feature resources and refresh through the normal rest flow.',
      'Innate racial spell charges are persisted as version-guarded race features and refresh through the normal short/long-rest flow.',
      'Flexible Casting updates Sorcery Points and normal spell-slot tracks atomically and consumes its combat bonus action.',
      'Temporary Flexible Casting slots above the normal spell-slot maximum are explicitly unsupported by the current slot model.',
    ],
