...
import {
  ...
  getCompendiumStats,
  reseedCompendium,
} from '../services/Open5eService.js';
import { requireAuth } from '../auth/middleware.js';
import { requireAdmin } from '../auth/admin.js';
import { isUploadStorageError, tokenUpload, validateAndSaveUpload } from './uploads.js';
...
function dbRowToMonster(row: Record<string, unknown>): CompendiumMonster {
  return {
    ...
    abilityScores: safeJsonParse(row.ability_scores, {
      ...
      wis: 10,
      cha: 10,
    }) as CompendiumMonster['abilityScores'],
    ...
    specialAbilities: safeJsonParse(
      row.special_abilities,
      []
    ) as CompendiumMonster['specialAbilities'],
    legendaryActions: safeJsonParse(
      row.legendary_actions,
      []
    ) as CompendiumMonster['legendaryActions'],
    ...
    tokenImageSource:
      (row.token_image_source as 'open5e' | 'uploaded' | 'ai-generated' | 'generated') ??
      'generated',
  };
}
...
function dbRowToItem(row: Record<string, unknown>): CompendiumItem & { rawJson?: unknown } {
  ...
  try {
    rawJson = row.raw_json ? JSON.parse(row.raw_json as string) : null;
  } catch {
    /* ignore */
  }
  return {
  ...
  if (!category || category === 'spells') {
    ...
    for (const s of spells) {
      results.push({
        ...
        name: s.name,
        category: 'spells',
        snippet: makeSnippet(s.description),
        level: s.level,
      });
    }
  }
  ...
  if (!category || category === 'items') {
    ...
    for (const i of items) {
      results.push({
        ...
        name: i.name,
        category: 'items',
        snippet: makeSnippet(i.description),
        rarity: i.rarity as CompendiumSearchResult['rarity'],
      });
    }
  }
...
router.get('/monsters/:slug', publicCompendiumCache, async (req: Request, res: Response) => {
  const { rows } = await pool.query('SELECT * FROM compendium_monsters WHERE slug = $1', [
    req.params.slug,
  ]);
  if (rows.length === 0) {
...
router.get(
  ...
  async (req: Request, res: Response) => {
    const { rows: mainRows } = await pool.query(
      'SELECT name FROM compendium_monsters WHERE slug = $1',
      [req.params.slug]
    );
...
router.get('/spells/:slug', publicCompendiumCache, async (req: Request, res: Response) => {
  const { rows } = await pool.query('SELECT * FROM compendium_spells WHERE slug = $1', [
    req.params.slug,
  ]);
  if (rows.length === 0) {
...
router.get('/items/:slug', publicCompendiumCache, async (req: Request, res: Response) => {
  const { rows } = await pool.query('SELECT * FROM compendium_items WHERE slug = $1', [
    req.params.slug,
  ]);
  if (rows.length === 0) {
...
router.get('/monsters', publicCompendiumCache, async (req: Request, res: Response) => {
  ...
  if (type) {
    where += ` AND type LIKE $${paramIdx++}`;
    params.push(`%${type}%`);
  }
  if (!isNaN(crMin)) {
    where += ` AND cr_numeric >= $${paramIdx++}`;
    params.push(crMin);
  }
  if (!isNaN(crMax)) {
    where += ` AND cr_numeric <= $${paramIdx++}`;
...
router.get('/spells', publicCompendiumCache, async (req: Request, res: Response) => {
  ...
  if (school) {
    where += ` AND school ILIKE $${paramIdx++}`;
    params.push(`%${school}%`);
  }
  if (classFilter) {
    where += ` AND classes ILIKE $${paramIdx++}`;
...
router.get('/items', publicCompendiumCache, async (req: Request, res: Response) => {
  ...
  if (rarity) {
    where += ` AND rarity LIKE $${paramIdx++}`;
    params.push(`%${rarity}%`);
  }
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const { rows } = await pool.query('SELECT name FROM compendium_monsters WHERE slug = $1', [
      slug,
    ]);
    if (rows.length === 0) {
      res.status(404).json({ error: 'Monster not found' });
    ...
    try {
      ...
      await pool.query('UPDATE compendium_monsters SET token_image_source = $1 WHERE slug = $2', [
        'uploaded',
        slug,
      ]);
      ...
      if (isUploadStorageError(err)) {
        console.error('[compendium:token-image] storage failed:', err);
