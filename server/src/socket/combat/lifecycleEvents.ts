...
import {
  combatStartSchema,
  combatAddCombatantSchema,
  combatReadyCheckSchema,
  combatReadyResponseSchema,
...
export function registerCombatLifecycle(io: Server, socket: Socket): void {
  socket.on(
    ...
    safeHandler(socket, async (data) => {
      ...
      if (ctx.player.role !== 'dm') {
        console.warn(
          '[COMBAT START] non-DM tried to start combat:',
          ctx.player.userId,
          ctx.player.role
        );
        return;
  ...
  socket.on(
    ...
    safeHandler(socket, async (data) => {
      ...
      const combatant = await CombatService.addCombatantAsync(
        ctx.room.sessionId,
        parsed.data.tokenId
      );
      if (!combatant) {
    ...
    safeHandler(socket, async () => {
      ...
      try {
        ...
        if (defeatedNames.length > 0 && pcCount > 0) {
          ...
          const lines: string[] = [];
          lines.push(
            `\uD83C\uDFC6 Combat ended \u2014 ${defeatedNames.length} defeated, ${pcCount} PC${pcCount === 1 ? '' : 's'}:`
          );
          for (const line of perMonster) lines.push(`  \u2022 ${line}`);
          lines.push(
            `  **Total: ${totalXp} XP \u2192 ${perPc} per PC**. Award via \`!xp <target\u2026> ${perPc}\`.`
          );
