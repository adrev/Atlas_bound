...
import { getPlayerBySocketId, playerIsDM, isTokenOwnerOrDM } from '../../utils/roomState.js';
import * as CombatService from '../../services/CombatService.js';
import * as ConditionService from '../../services/ConditionService.js';
import {
  combatConditionSchema,
  conditionWithMetaSchema,
  damageSideEffectsSchema,
  concentrationDroppedSchema,
...
export function registerCombatConditions(io: Server, socket: Socket): void {
  ...
  socket.on(
    ...
    safeHandler(socket, async (data) => {
      ...
      if (ctx.player.role !== 'dm') {
        const VOLUNTARY_CONDITIONS = new Set(['concentrating', 'dodging', 'raging', 'hiding']);
  ...
  socket.on(
    ...
    safeHandler(socket, async (data) => {
      ...
      if (!isDM) {
        ...
        const DESTRUCTIVE_CONDITIONS = new Set(['dead', 'unconscious', 'petrified', 'stable']);
      ...
      // the metadata map. Returns any other tokens whose condition state
      // changed as a side effect (released grapples, dropped caster-linked
      // effects) so clients repaint those badges too.
  ...
  socket.on(
    ...
    safeHandler(socket, async (data: unknown) => {
      ...
      if (
        !Number.isFinite(parsed.data.damageAmount) ||
        parsed.data.damageAmount < 0 ||
        parsed.data.damageAmount > 9999
      )
        return;
