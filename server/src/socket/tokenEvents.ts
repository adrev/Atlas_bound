...
async function emitLatestTokenConflict(
  io: Server,
  socketId: string,
  tokenId: string
): Promise<Token | null> {
  ...
  if (!row) return null;
  const latestToken = rowToToken(row);
  io.to(socketId).emit('map:token-conflict', { token: latestToken });
  return latestToken;
}
...
export function registerTokenEvents(io: Server, socket: Socket): void {
  socket.on(
    ...
    safeHandler(socket, async (data) => {
      ...
      const { tokenId, x, y, expectedVersion } = parsed.data;
      ...
      const updateParams: unknown[] = [x, y, tokenId];
      let updateSql = 'UPDATE tokens SET x = $1, y = $2 WHERE id = $3';
      if (expectedVersion !== undefined) {
        updateParams.push(expectedVersion);
        updateSql += ' AND version = $4';
      }
      updateSql += ' RETURNING version';
      const { rows: updatedRows } = await pool.query(updateSql, updateParams);
      if (updatedRows.length === 0) {
        const latestToken = await emitLatestTokenConflict(io, socket.id, tokenId);
        if (latestToken && ctx.room.tokens.has(tokenId)) {
          ctx.room.tokens.set(tokenId, latestToken);
        }
        return;
      }
      const nextVersion = Number(updatedRows[0].version ?? token.version ?? 1);
      ...
      broadcastEventToSockets(
        ...
        ctx.room,
        'map:token-moved',
        { tokenId, x, y, mapId: token.mapId, version: nextVersion },
        moveRecipients,
  ...
  socket.on(
    ...
    safeHandler(socket, async (data) => {
      ...
      const { tokenId, changes, expectedVersion } = parsed.data;
      let token = ctx.room.tokens.get(tokenId);
      let tokenMapId: string | null = null;
      if (token) {
      ...
      params.push(tokenId);
      let updateSql = `UPDATE tokens SET ${setClauses.join(', ')} WHERE id = $${paramIdx}`;
      if (expectedVersion !== undefined) {
      ...
      if (updatedRows.length === 0) {
        const latestConflictToken = await emitLatestTokenConflict(io, socket.id, tokenId);
        if (latestConflictToken && ctx.room.tokens.has(tokenId)) {
          ctx.room.tokens.set(tokenId, latestConflictToken);
        }
        return;
      }
      const nextVersion = Number(updatedRows[0].version ?? token.version ?? 1);
      const broadcastChanges = { ...changes, version: nextVersion } as Partial<Token>;
      if (ctx.room.tokens.has(tokenId)) Object.assign(token, broadcastChanges);
      ...
      if (tokenMapId) {
        ...
        io.to(ctx.room.sessionId).emit('map:token-updated', { tokenId, changes: broadcastChanges });
      }
    })
  );
}
