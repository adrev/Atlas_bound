...
export function registerMapEvents(io: Server, socket: Socket): void {
  socket.on(
    ...
    safeHandler(socket, async (data) => {
      ...
      // Same guard as map:activate-for-players: this legacy load path
      // clears room.tokens, which orphans every combatant mid-fight.
      // (map:preview-load remains available during combat — it doesn't
      // touch room.tokens.)
      if (ctx.room.combatState?.active) {
        socket.emit('session:error', {
          message:
            "End combat before changing maps — the active fight's combatants live on this map.",
        });
        return;
      }
      ...
      if (!ctx.room.playerMapId) {
        ...
        try {
          await pool.query('UPDATE sessions SET player_map_id = $1 WHERE id = $2', [
            mapId,
            ctx.room.sessionId,
          ]);
        } catch (err) {
          console.warn('[map:load] player_map_id update failed:', err);
        }
      }
      ...
      await pool.query('UPDATE sessions SET current_map_id = $1 WHERE id = $2', [
        mapId,
        ctx.room.sessionId,
      ]);
  ...
  socket.on(
    ...
    safeHandler(socket, async (data) => {
      ...
      const tier =
        typeof raw?.ambientLight === 'string' && VALID.includes(raw.ambientLight as string)
      ...
      let i = 1;
      if (tier) {
        updates.push(`ambient_light = $${i++}`);
        values.push(tier);
      }
      if (opacity === null) updates.push(`ambient_opacity = NULL`);
      else if (typeof opacity === 'number') {
        updates.push(`ambient_opacity = $${i++}`);
        values.push(opacity);
      }
