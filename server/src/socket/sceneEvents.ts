...
const MAP_SUMMARY_SELECT = `
  SELECT m.id, m.name, m.image_url, m.thumbnail_url, m.width, m.height, m.grid_size,
         ...
         -- instead of nuking the whole map:list-result with a SQL error.
         COALESCE(jsonb_array_length(NULLIF(m.walls, '')::jsonb), 0) AS wall_count,
         (SELECT COUNT(*) FROM map_zones z WHERE z.map_id = m.id) AS zone_count
  FROM maps m
...
async function broadcastMapListToDMs(
  ...
  room: {
    ...
    players: Map<string, { role: string; socketId: string }>;
  }
): Promise<void> {
...
export function registerSceneEvents(io: Server, socket: Socket): void {
  socket.on(
    ...
    safeHandler(socket, async () => {
      ...
      console.log(
        `[SCENE] map:list (${ctx.player.role}) → ${maps.length} maps, ribbon=${ctx.room.playerMapId ?? 'null'}`
      );
  ...
  socket.on(
    ...
    safeHandler(socket, async (data) => {
      ...
      // Moving the ribbon clears room.tokens for the new map. During an
      // active fight that orphans every NPC combatant (their tokens stay
      // on the old map): each monster turn auto-skips, announcements show
      // '???', and the encounter zombifies until combat is force-ended.
      // Block the move and tell the DM to end combat first.
      if (ctx.room.combatState?.active) {
        socket.emit('session:error', {
          message:
            "End combat before moving the party — the active fight's combatants live on this map.",
        });
        return;
      }
  ...
  socket.on(
    ...
    safeHandler(socket, async (data) => {
      ...
      if (ctx.room.playerMapId === mapId) {
        socket.emit('session:error', {
          message:
            'Cannot delete the map the players are currently on. Move the player ribbon to another map first.',
        });
        return;
      }

      const { rows: mapRows } = await pool.query(
        'SELECT id FROM maps WHERE id = $1 AND session_id = $2',
        [mapId, ctx.room.sessionId]
      );
      if (mapRows.length === 0) {
        socket.emit('session:error', { message: 'Map not found in this session' });
  ...
  socket.on(
    ...
    safeHandler(socket, async (data) => {
      ...
      if (owned.size !== mapIds.length) {
        socket.emit('session:error', {
          message: 'Reorder list contains maps from another session',
        });
        return;
      }
