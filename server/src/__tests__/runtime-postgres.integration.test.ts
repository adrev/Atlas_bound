...
async function seedImportantState(ids: FixtureIds): Promise<void> {
  await connection.default.query(
    `UPDATE characters SET hit_points = 9, max_hit_points = 30, temp_hit_points = 2,
     armor_class = 15, speed = 30, exhaustion_level = 2,
     death_saves = '{"successes":1,"failures":2}' WHERE id = $1`,
    [ids.characterId]
  );
...
describe.skipIf(!database)('REAL PostgreSQL runtime durability', () => {
  ...
  it('reloads authoritative main character stats and privacy after a REST commit without fanout', async () => {
    const ids = await fixture();
    await runtime.withSessionRuntime(ids.sessionId, async () => seedImportantState(ids));
    await connection.rawPool.query(
      `UPDATE characters SET hit_points = 18, max_hit_points = 30, temp_hit_points = 4,
       armor_class = 16, speed = 35, death_saves = '{"successes":0,"failures":0}' WHERE id = $1`,
      [ids.characterId]
    );
    await connection.rawPool.query('UPDATE sessions SET settings = $2 WHERE id = $1', [
      ids.sessionId,
      JSON.stringify({ showPlayersToPlayers: true, showCreatureStatsToPlayers: true }),
    ]);
    ...
    await runtime.withSessionRuntime(ids.sessionId, async () => {
      ...
      expect(room.showPlayersToPlayers).toBe(true);
      expect(room.showCreatureStatsToPlayers).toBe(true);
      expect(room.actionEconomies.get(ids.tokenId)?.action).toBe(true);
    });
    const saved = (await durableRows(ids)).room.state.values;
    expect(saved.combatState.combatants[0].hp).toBe(18);
  });

  it('serializes main REST sheet edits with a running room transaction and saves fanout', async () => {
    const ids = await fixture();
    await runtime.withSessionRuntime(ids.sessionId, async () => seedImportantState(ids));
    const entered = deferred();
    const release = deferred();
    const held = runtime.withSessionRuntime(ids.sessionId, async () => {
      entered.resolve();
      await release.promise;
    });
    await entered.promise;
    let committed = false;
    const edit = connection.rawPool
      .query('UPDATE characters SET hit_points = 23 WHERE id = $1', [ids.characterId])
      .then(() => {
        committed = true;
      });
    try {
    ...
    await held;
    await edit;
    const { fanoutCharacterUpdateAcrossRooms } =
      await import('../services/CharacterUpdateService.js');
    const io = { to: () => ({ emit: () => true }) } as unknown as Server;
    await fanoutCharacterUpdateAcrossRooms(io, ids.characterId, ids.userId, { hitPoints: 23 });
  ...
  it('keeps main XP savepoint commits and their success messages inside the runtime transaction', async () => {
    ...
    const command = async () => {
      ...
      rooms.addPlayerToRoom(ids.sessionId, player);
      await tryHandleChatCommand(
        io,
        { room, player },
        `!xp ${room.tokens.get(ids.tokenId)!.name} 20`
      );
    };
    await expect(
      runtime.withSessionRuntime(ids.sessionId, async () => {
        ...
        expect(
          (
            await connection.rawPool.query('SELECT experience FROM characters WHERE id = $1', [
              ids.characterId,
            ])
          ).rows[0].experience
        ).toBe(0);
        expect(emitted).toEqual([]);
        void connection.default.query('SELECT 1/0').catch(() => {});
      })
    ).rejects.toThrow();
    expect(emitted).toEqual([]);
    expect(
    ...
    await runtime.withSessionRuntime(ids.sessionId, command);
    expect(
      (
        await connection.rawPool.query('SELECT experience FROM characters WHERE id = $1', [
          ids.characterId,
