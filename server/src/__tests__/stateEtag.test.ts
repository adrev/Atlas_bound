...
// role, viewing map, privacy policy, event cursor, or the time bucket), and stay stable
...
const base = {
  ...
  userId: 'u1',
  isDM: false,
  viewingMapId: 'map-1' as string | null,
...
describe('stateSnapshotEtag', () => {
  ...
  it('changes immediately when creature-sheet sharing changes', () => {
    expect(stateSnapshotEtag(base)).not.toBe(
      stateSnapshotEtag({ ...base, showCreatureStatsToPlayers: true })
    );
  });

  it('changes immediately when party-sheet sharing changes', () => {
    expect(stateSnapshotEtag(base)).not.toBe(
      stateSnapshotEtag({ ...base, showPlayersToPlayers: true })
    );
  });

  it('rolls over when the time bucket flips (bounds unwrapped-broadcast staleness)', () => {
    expect(stateSnapshotEtag(base)).not.toBe(
      stateSnapshotEtag({ ...base, now: base.now + 61_000 })
    );
  });

  it('treats a null viewing map distinctly from a named map', () => {
    expect(stateSnapshotEtag({ ...base, viewingMapId: null })).not.toBe(
      stateSnapshotEtag({ ...base, viewingMapId: 'map-1' })
    );
  });
