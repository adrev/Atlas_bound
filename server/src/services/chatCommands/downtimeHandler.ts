...
async function handleCraft(c: ChatCommandContext): Promise<boolean> {
  ...
  if (
    maybeHelpers !== null &&
    parts.length >= 3 &&
    Number.isFinite(parseInt(parts[parts.length - 2], 10))
  ) {
...
const MULTICLASS_REQS: Record<
  string,
  { need: Array<Array<'str' | 'dex' | 'con' | 'int' | 'wis' | 'cha'>>; note: string }
> = {
...
async function handleMulticlass(c: ChatCommandContext): Promise<boolean> {
  ...
  const lookup = (k: string): number =>
    Number(loaded.scores?.[k] ?? loaded.scores?.[k.toLowerCase()]) || 0;
...
async function handleEncumbrance(c: ChatCommandContext): Promise<boolean> {
  ...
  lines.push(
    `   • Heavily enc.: **${heavyAt + 1}–${maxLift} lb** (speed −20 ft, disadvantage on STR / DEX / CON ability checks, attack rolls, and saves)`
  );
  lines.push(
    `   • Max lift:     **${maxLift} lb** (PHB standard carrying capacity — no penalties)`
  );
...
async function handleCurrency(c: ChatCommandContext): Promise<boolean> {
  ...
  lines.push(`   Rates: 10 cp = 1 sp · 5 sp = 1 ep · 2 ep = 1 gp · 10 gp = 1 pp.`);
