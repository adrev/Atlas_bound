...
const WILD_MAGIC_TABLE: Array<{ range: [number, number]; desc: string }> = [
  ...
  {
    range: [5, 6],
    desc: 'A modron chosen and controlled by the DM appears in an unoccupied space within 5 ft, then disappears 1 min later.',
  },
  {
    range: [7, 8],
    desc: 'You cast Fireball (DC=yours) as a 3rd-level spell centered on yourself.',
  },
  ...
  {
    range: [15, 16],
    desc: 'For the next minute, you regain 5 HP at the start of each of your turns.',
  },
  {
    range: [17, 18],
    desc: 'You grow a long beard made of feathers that remains until you sneeze — at that point the feathers fly out.',
  },
  { range: [19, 20], desc: 'You cast Grease centered on yourself.' },
  {
    range: [21, 22],
    desc: 'Creatures have disadvantage on saving throws against the next spell you cast in the next minute that involves a save.',
  },
  { range: [23, 24], desc: 'Your skin turns bright blue for 24 hours; Remove Curse removes it.' },
  {
    range: [25, 26],
    desc: 'A 3rd eye appears on your forehead for 1 min — you have advantage on sight-based WIS(Perception) checks.',
  },
  {
    range: [27, 28],
    desc: 'For the next minute, all your spells with a casting time of 1 action become bonus-action spells.',
  },
  {
    range: [29, 30],
    desc: 'You teleport up to 60 ft to an unoccupied space of your choice you can see.',
  },
  {
    range: [31, 32],
    desc: 'You are transported to the Astral Plane until the end of your next turn. Return to the space you occupied (or nearest unoccupied).',
  },
  {
    range: [33, 34],
  ...
  {
    range: [35, 36],
    desc: 'Roll a d10. Your age changes by that many years; on even, older. On odd, younger (minimum 1).',
  },
  {
    range: [37, 38],
    desc: '1d6 flumphs controlled by the DM appear in unoccupied spaces within 60 ft and are frightened of you. Disappear 1 min later.',
  },
  { range: [39, 40], desc: 'You regain 2d10 HP.' },
  {
    range: [41, 42],
    desc: 'You turn into a potted plant until the start of your next turn. Incapacitated + vulnerable to all damage. If reduced to 0 HP, pot breaks + form returns.',
  },
  {
    range: [43, 44],
    desc: 'For the next minute, you can teleport up to 20 ft as a bonus action on each turn.',
  },
  { range: [45, 46], desc: 'You cast Levitate on yourself.' },
  {
    range: [47, 48],
    desc: 'A unicorn controlled by the DM appears in an unoccupied space within 5 ft, disappears 1 min later.',
  },
  {
    range: [49, 50],
  ...
  {
    range: [51, 52],
    desc: 'A spectral shield hovers near you for the next minute, granting +2 AC and immune to Magic Missile.',
  },
  {
    range: [53, 54],
  ...
  {
    range: [57, 58],
    desc: "For the next minute, any flammable object you touch that isn't being worn or carried ignites.",
  },
  ...
  {
    range: [65, 66],
    desc: 'Up to three creatures you choose within 30 ft take 4d10 lightning damage.',
  },
  {
    range: [67, 68],
  ...
  {
    range: [69, 70],
    desc: 'Each creature within 30 ft becomes invisible for the next minute; invisibility ends on attack or cast.',
  },
  ...
  {
    range: [75, 76],
    desc: 'You glow with bright light in a 30-ft radius for the next minute. Any creature that ends turn within 5 ft of you is blinded until end of its next turn.',
  },
  {
    range: [77, 78],
    desc: 'You cast Polymorph on yourself. If you fail the WIS save you become a sheep for the duration.',
  },
  {
    range: [79, 80],
    desc: 'Illusory butterflies + flower petals flutter in the air within 10 ft of you for the next minute.',
  },
  { range: [81, 82], desc: 'You can take one additional action immediately.' },
  {
    range: [83, 84],
    desc: 'Each creature within 30 ft takes 1d10 necrotic damage; you regain HP equal to total damage dealt.',
  },
  { range: [85, 86], desc: 'You cast Mirror Image.' },
  { range: [87, 88], desc: 'You cast Fly on a random creature within 60 ft.' },
  {
    range: [89, 90],
    desc: 'You become invisible for the next minute; ends if you attack or cast.',
  },
  {
    range: [91, 92],
    desc: 'If you die within the next minute, you return to life 1 min later with 1 HP as if Reincarnated.',
  },
  {
    range: [93, 94],
    desc: 'Your size changes by one category for the next minute. Roll 1d4: 1 tiny, 2 small, 3 large, 4 huge.',
  },
  {
    range: [95, 96],
    desc: 'You and all creatures within 30 ft gain vulnerability to piercing damage for the next minute.',
  },
...
async function handleSurge(c: ChatCommandContext): Promise<boolean> {
  ...
  const { rows } = await pool.query('SELECT class, name FROM characters WHERE id = $1', [
    caller.characterId,
  ]);
  const row = rows[0] as Record<string, unknown> | undefined;
  const classLower = String(row?.class || '').toLowerCase();
...
async function handleWard(c: ChatCommandContext): Promise<boolean> {
  ...
  const { rows } = await pool.query(
    'SELECT class, level, ability_scores, name FROM characters WHERE id = $1',
    [caller.characterId]
  );
  const row = rows[0] as Record<string, unknown> | undefined;
  const classLower = String(row?.class || '').toLowerCase();
  ...
  if (sub === 'status' || sub === '') {
    const w = characterFeatures(caller.characterId).arcaneWard;
    if (!w) {
      whisperToCaller(
        c.io,
        c.ctx,
        `🛡 ${charName} Arcane Ward: not yet summoned. Cast an abjuration 1+ to init (max ${wardMax}).`
      );
    } else {
  ...
  if (sub === 'reset') {
    delete characterFeatures(caller.characterId).arcaneWard;
    broadcastSystem(c.io, c.ctx, `🛡 ${charName} Arcane Ward dissipates (long rest).`);
    return true;
  }
  if (sub === 'init') {
    const w = { current: wardMax, max: wardMax };
    characterFeatures(caller.characterId).arcaneWard = w;
    broadcastSystem(
      c.io,
      c.ctx,
      `🛡 ${charName}'s Arcane Ward springs up — ${wardMax}/${wardMax} HP.`
    );
    return true;
  }
  ...
  if (!w) {
    whisperToCaller(
      c.io,
      c.ctx,
      '!ward: no recorded ward. Use !ward init when summoned, then reconcile its HP.'
    );
    return true;
  }
  ...
  if (sub === 'heal' || sub === 'recharge') {
    ...
    broadcastSystem(
      c.io,
      c.ctx,
      `🛡 ${charName}'s Arcane Ward recharges ${w.current - prior} → ${w.current}/${w.max}.`
    );
    return true;
  }
...
async function handleSculpt(c: ChatCommandContext): Promise<boolean> {
  ...
  if (!hasSculpt) {
    whisperToCaller(
      c.io,
      c.ctx,
      `!sculpt: ${caller.name} doesn't have Sculpt Spells (requires School of Evocation).`
    );
    return true;
  }
  const exempt = 1 + lvl;
