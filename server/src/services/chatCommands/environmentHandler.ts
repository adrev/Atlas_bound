...
async function handleUnderwater(c: ChatCommandContext): Promise<boolean> {
  ...
  if (sub === 'on' || sub === 'start' || sub === 'enable') {
    sessionFeatures(c.ctx.room.sessionId).underwater = true;
    broadcastSystem(
      c.io,
      c.ctx,
  ...
  if (sub === 'off' || sub === 'end' || sub === 'disable') {
    sessionFeatures(c.ctx.room.sessionId).underwater = false;
    broadcastSystem(
      c.io,
      c.ctx,
      '🏞 Underwater combat deactivated — normal movement + attack rules resume.'
    );
    return true;
  }
  const active = sessionFeatures(c.ctx.room.sessionId).underwater === true;
  whisperToCaller(
    c.io,
    c.ctx,
...
async function handleMount(c: ChatCommandContext): Promise<boolean> {
  ...
  if (parts.length < 2) {
    whisperToCaller(c.io, c.ctx, '!mount: usage `!mount <rider> <mount> [controlled|independent]`');
    return true;
  }
  ...
  const mounts = (sessionFeatures(c.ctx.room.sessionId).mountLinks ??= {});
  mounts[rider.id] = {
...
const CHASE_URBAN: Array<{ roll: number; text: string }> = [
  {
    roll: 1,
    text: '**Horse-drawn carriage** blocks the way. DC 15 DEX save or fall prone (10 ft of movement to stand).',
  },
  {
    roll: 2,
    text: '**Crate or barrel stack** — DC 10 DEX save, fail = 10 ft of difficult terrain.',
  },
  {
    roll: 3,
    text: '**Slippery cobblestones** (rain / fish guts). DC 10 DEX save, fail = fall prone.',
  },
  ...
  {
    roll: 6,
    text: '**Window / door** in your path. DC 10 STR save to force through; spend 15 ft of movement.',
  },
  {
    roll: 7,
    text: "**Pickpocket** brushes past. DC 10 WIS save or don't notice; on success DC 15 DEX to grab them.",
  },
  {
    roll: 8,
    text: '**Passerby helps** the fleeing party they root for. DC 10 STR save to push through.',
  },
  {
    roll: 9,
    text: '**Patrol** on the corner. Roll DEX (Stealth) vs their passive Perception to slip by unnoticed.',
  },
  { roll: 10, text: '**Guarded gate** ahead. Bribe, bluff, or bash — 20 ft of movement cost.' },
  {
    roll: 11,
    text: '**Sewer grate** loose — DC 10 DEX save, fail = leg stuck, prone, one turn to free.',
  },
  {
    roll: 12,
    text: '**Stray cat** — no save needed, purely flavor (DC 10 Animal Handling if you pause).',
  },
  {
    roll: 13,
    text: '**Narrow alley** — Large creatures have disadvantage on DEX saves to avoid obstacles.',
  },
  {
    roll: 14,
    text: '**Wash line strung across street** at neck height. DC 10 DEX save, fail = prone.',
  },
  ...
  {
    roll: 17,
    text: '**Crowded market.** Dash requires DC 10 STR or DEX check; fail = 10 ft of movement lost.',
  },
  {
    roll: 18,
    text: '**Wagon wheels** loose. DC 10 DEX save, fail = 1d4 bludgeoning as rolling debris hits.',
  },
];
...
const CHASE_WILDERNESS: Array<{ roll: number; text: string }> = [
  {
    roll: 1,
    text: '**Swarm of insects** in path. DC 10 CON save or 1d4 piercing + disadvantage for next move.',
  },
  { roll: 2, text: '**Steep slope** up or down. Movement costs doubled until off slope.' },
  {
    roll: 3,
    text: '**Thick bramble.** DC 10 STR save to push through; fail = 1d4 slashing + restrained until next turn.',
  },
  ...
  {
    roll: 6,
    text: '**Animal burrow** — DC 10 DEX save, fail = leg caught, prone + speed halved until freed.',
  },
  { roll: 7, text: '**Dense fog** rolls in. Heavily obscured beyond 5 ft for the next round.' },
  {
    roll: 8,
    text: '**Creek / stream.** DC 10 STR save to cross at speed; fail = difficult terrain.',
  },
  ...
  {
    roll: 12,
    text: '**Beehive or hornet swarm** disturbed. DC 10 CON save or 1d4 poison + disadvantage for 1 turn.',
  },
  {
    roll: 13,
    text: '**Wild deer / rabbit** bolts across path. DC 10 WIS save or lose focus (disadvantage on next attack).',
  },
  { roll: 14, text: '**Muddy patch** — 10 ft difficult terrain.' },
  {
    roll: 15,
    text: '**Fallen tree gap** — DC 12 STR (Athletics) to climb over; costs 15 ft of movement.',
  },
  {
    roll: 16,
    text: '**Bird flock** erupts — DC 10 WIS save or disadvantage on next Perception check.',
  },
...
async function handleChase(c: ChatCommandContext): Promise<boolean> {
  ...
  const entry = table.find((e) => e.roll === d20) ?? {
    roll: d20,
    text: `No complication — fleet foot or lucky break (roll ${d20}).`,
  };
  const caller = Array.from(c.ctx.room.tokens.values()).find(
    (t) => t.ownerUserId === c.ctx.player.userId
  );
