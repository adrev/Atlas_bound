...
const CONTEXT_CONDITIONS: Condition[] = [
  'blinded',
  'charmed',
  'deafened',
  'frightened',
...
function buildAura(radiusFeet: number, current?: TokenAura | null): TokenAura {
  return {
    radiusFeet,
    color: current?.color || '#d4a843',
    opacity: current?.opacity || 0.2,
    shape: current?.shape || 'circle',
  };
}

function setAuraColor(current: TokenAura | null | undefined, color: string): TokenAura {
  return current ? { ...current, color } : { radiusFeet: 10, color, opacity: 0.2, shape: 'circle' };
}
...
export function TokenContextMenu() {
  ...
  const scores = character ? parse<Partial<AbilityScores>>(character.abilityScores, {}) : {};
  const hp = character?.hitPoints ?? 0;
  const maxHp = character?.maxHitPoints ?? 0;
  const ac = character?.armorClass ?? 10;
  const inventory = character ? parse<InventoryItem[]>(character.inventory, []) : [];
  ...
  const close = () => {
    useMapStore.getState().setContextMenu(null, null);
    setSubMenu(null);
    setHpInput('');
  };
  ...
  const menuX = Math.min(
    contextPos.x,
    typeof window !== 'undefined' ? window.innerWidth - 240 : 600
  );
  const menuY = Math.min(
    contextPos.y,
    typeof window !== 'undefined' ? window.innerHeight - 500 : 300
  );
      ...
      >
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    ...
                    if (!isNaN(v) && v > 0 && token.characterId) {
                      emitCharacterUpdate(token.characterId, { hitPoints: Math.max(0, hp - v) });
                      close();
                    }
                  }
                }}
            ...
            <div style={{ display: 'flex', gap: 3 }}>
              ...
              <Btn
                ...
                onClick={() => {
                  const v = parseInt(hpInput);
                  if (!isNaN(v) && v > 0 && token.characterId) {
                    emitCharacterUpdate(token.characterId, { hitPoints: Math.min(maxHp, hp + v) });
                    close();
                  }
                }}
              />
            ...
            <div style={{ display: 'flex', gap: 2, marginTop: 4 }}>
                ...
                <MiniBtn
                  ...
                  onClick={() => {
                    if (token.characterId) {
                      emitCharacterUpdate(token.characterId, { hitPoints: Math.max(0, hp - n) });
                      close();
                    }
                  }}
                />
              ...
              {[1, 5, 10].map((n) => (
                <MiniBtn
                  ...
                  onClick={() => {
                    if (token.characterId) {
                      emitCharacterUpdate(token.characterId, {
                        hitPoints: Math.min(maxHp, hp + n),
                      });
                      close();
                    }
                  }}
        ...
        {subMenu === 'attack' && (
          <div style={{ padding: '8px 12px', borderBottom: `1px solid ${C.border}` }}>
            {weapons.length > 0 ? (
              weapons.map((w, i) => {
                const isFinesse = (w.properties || []).some((p: string) =>
                  p.toLowerCase().includes('finesse')
                );
                const isRanged = (w.properties || []).some((p: string) =>
                  p.toLowerCase().includes('range')
                );
                const atkMod =
                  (isRanged ? dexMod : isFinesse ? Math.max(strMod, dexMod) : strMod) + profBonus;
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      gap: 4,
                      padding: '3px 0',
                      borderBottom: i < weapons.length - 1 ? `1px solid ${C.borderDim}` : 'none',
                    }}
                  >
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 12, fontWeight: 600 }}>{w.name}</div>
                      <div style={{ fontSize: 9, color: C.textMuted }}>
                        +{atkMod} hit • {w.damage || '1d4'}+{dmgMod}
                      </div>
                    </div>
                    <MiniBtn
                      label="ATK"
                      color={C.red}
                      onClick={() => {
                        emitRoll(`1d20+${atkMod}`, `${token.name} ${w.name}`);
                        close();
                      }}
                    />
                    <MiniBtn
              ...
              <div style={{ display: 'flex', gap: 3 }}>
                <Btn
                  label={`Attack (d20+${Math.max(strMod, dexMod) + profBonus})`}
                  color={C.red}
                  onClick={() => {
                    emitRoll(
                      `1d20+${Math.max(strMod, dexMod) + profBonus}`,
                      `${token.name} Attack`
                    );
                    close();
                  }}
                />
                <Btn
                  label={`Damage (1d6+${strMod})`}
                  color={C.gold}
                  onClick={() => {
                    emitRoll(`1d6+${strMod}`, `${token.name} Damage`);
                    close();
                  }}
                />
              </div>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const updated = active
                        ...
                        : [...current, cond];
                      emitTokenUpdate(token.id, { conditions: updated });
                      close();
                    }}
                    style={{
                      padding: '2px 6px',
          ...
          <div style={{ padding: '8px 12px', borderBottom: `1px solid ${C.border}` }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3, marginBottom: 8 }}>
              {[5, 10, 15, 20, 30, 60].map((r) => (
                <button
                  key={r}
                  onClick={() => {
                    const current = token.aura;
                    emitTokenUpdate(token.id, {
                      aura: buildAura(r, current),
                    });
                    close();
                  }}
                  style={{
                    ...
                    borderRadius: 3,
                    cursor: 'pointer',
                    background: token.aura?.radiusFeet === r ? 'rgba(212,168,67,0.3)' : C.bgHover,
                    border: `1px solid ${token.aura?.radiusFeet === r ? C.gold : C.border}`,
                    color: token.aura?.radiusFeet === r ? C.gold : C.textSec,
                    fontWeight: 600,
                  }}
                >
                  {r} ft
            ...
            <div style={{ display: 'flex', gap: 4, marginBottom: 8 }}>
              {[
                ...
                <button
                  key={preset.label}
                  onClick={() => {
                    emitTokenUpdate(token.id, { aura: setAuraColor(token.aura, preset.color) });
                    close();
                  }}
                  style={{
                    width: 24,
                    ...
                    border: `2px solid ${token.aura?.color === preset.color ? '#fff' : 'transparent'}`,
                    opacity: 0.8,
                  }}
                  title={preset.label}
                />
            ...
            {token.aura && (
              <Btn
                ...
                color={C.red}
                onClick={() => {
                  emitTokenUpdate(token.id, { aura: null });
                  close();
                }}
              />
            )}
        ...
        {subMenu === null && (
          <div>
            ...
            <Item
              ...
              onClick={() => {
                emitPing(
                  token.x + (gridSize * token.size) / 2,
                  token.y + (gridSize * token.size) / 2
                );
                close();
              }}
            />
            ...
            {isDM && (
              <Item
                icon="🔮"
                label={token.aura ? 'Aura (active)' : 'Aura'}
                onClick={() => setSubMenu('aura')}
                hasArrow
              />
            )}
            {isDM && (
              <Item
                ...
                label={
                  token.hasLight
                    ? `Light (${Math.round((token.lightRadius / gridSize) * 5)} ft)`
                    : 'Light source'
                }
                onClick={() => setSubMenu('light')}
                hasArrow
              />
            )}
            <Item
              ...
              onClick={async () => {
                ...
                const isNPC =
                  !token.ownerUserId ||
                  (character?.source === 'manual' && character?.userId === 'npc');
              ...
              /^(Light|Dancing Lights) \(/.test(token.name) && (
                <>
                  ...
                  <Item
                    ...
                    onClick={() => {
                      emitTokenRemove(contextTokenId);
                      close();
                    }}
                    danger
                  />
              ...
              <>
                ...
                {!combatActive && Object.keys(tokens).length > 0 && (
                  <Item
                    ...
                    label="Start Combat"
                    onClick={() => {
                      emitStartCombat(Object.keys(tokens));
                      close();
                    }}
                  />
                )}
                {combatActive && (
                  <Item
                    ...
                    label="End Combat"
                    onClick={() => {
                      import('../../socket/emitters').then(({ emitEndCombat: ec }) => ec());
                      close();
                    }}
                  />
                )}
                ...
                {/* Layer / visibility */}
                <Item
                  icon={token.visible ? '👁️' : '🙈'}
                  label={token.visible ? 'Hide Token' : 'Show Token'}
                  onClick={() => {
                    emitTokenUpdate(token.id, { visible: !token.visible });
                    close();
                  }}
                />
                <Item
                  icon={useMapStore.getState().lockedTokenIds.has(token.id) ? '🔓' : '🔒'}
                  label={
                    useMapStore.getState().lockedTokenIds.has(token.id)
                      ? 'Unlock Position'
                      : 'Lock Position'
                  }
                  onClick={() => {
                    useMapStore.getState().toggleLockToken(token.id);
                    close();
                  }}
                />
                ...
                <div style={{ padding: '4px 12px', display: 'flex', alignItems: 'center', gap: 4 }}>
                  ...
                  {[
                    ...
                    <button
                      key={s.label}
                      onClick={() => {
                        emitTokenUpdate(token.id, { size: s.size });
                        close();
                      }}
                      style={{
                        padding: '2px 6px',
                        ...
                        color: token.size === s.size ? C.gold : C.textSec,
                        fontWeight: 600,
                      }}
                    >
                      {s.label}
                ...
                <Item
                  ...
                  label="Copy Token"
                  onClick={() => {
                    useMapStore.getState().copyToken(token);
                    close();
                  }}
                />
                <Item
                  ...
                  onClick={async () => {
                    ...
                    if (!charId) {
                      try {
                        ...
                        const comp: CompendiumMonster | null = compResp.ok
                          ? await compResp.json()
                          : null;
                        ...
                        if (createResp.ok) {
                          ...
                          charId = data.id;
                          emitTokenUpdate(token.id, { characterId: charId });
                          useMapStore.getState().updateToken(token.id, { characterId: charId });
                        }
                      } catch {}
                    }
                    if (charId) {
                ...
                {(['str', 'dex', 'con', 'int', 'wis', 'cha'] as const).map((ab) => (
                  <div key={ab} style={{ textAlign: 'center' }}>
                    ...
                    <div
                      ...
                      onClick={() => {
                        emitRoll(
                          `1d20+${abilityModifier(scores[ab] || 10) >= 0 ? '+' : ''}${abilityModifier(scores[ab] || 10)}`,
                          `${token.name} ${ab.toUpperCase()} Check`
                        );
                        close();
                      }}
                      title={`Roll ${ab.toUpperCase()} check`}
                    >
                      {abilityModifier(scores[ab] || 10) >= 0 ? '+' : ''}
                      {abilityModifier(scores[ab] || 10)}
                    </div>
                  </div>
                ))}
              </div>
...
function Item({
  ...
  return (
    ...
    >
      <span style={{ fontSize: 13, width: 18, textAlign: 'center' }} aria-hidden>
        {icon}
      </span>
      <span style={{ flex: 1 }}>{label}</span>
      {hasArrow && (
        <span style={{ fontSize: 10, color: C.textMuted }} aria-hidden>
          ›
        </span>
      )}
    </button>
