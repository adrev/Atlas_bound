...
export function MapContextMenu() {
  ...
  const myHeroAlreadyOnMap =
    !!myCharacter && Object.values(tokens).some((t) => t.characterId === myCharacter.id);
  ...
  const menuX = Math.min(
    menu.screenX,
    typeof window !== 'undefined' ? window.innerWidth - 220 : 600
  );
    ...
    <>
      ...
      >
        ...
        {isDM && useMapStore.getState().copiedToken && (
          <Item
            icon="📋"
            label={`Paste ${useMapStore.getState().copiedToken!.name}`}
            onClick={() => {
              const copied = useMapStore.getState().copiedToken;
              if (!copied || !currentMap) {
                close();
                return;
              }
              emitTokenAdd({
        ...
        {canPlaceMyHero && (
          <Item
            icon="🧍"
            label={`Place ${myCharacter!.name} Here`}
            onClick={() => {
              if (!currentMap || !myCharacter) {
                close();
                return;
              }
              emitTokenAdd({
        ...
        {isDM && (
          <>
            ...
            {/* Manual fog brush — reveal or re-fog player-visible areas */}
            <Item
              icon="🌫️"
              label="Reveal Fog Brush"
              onClick={() => {
