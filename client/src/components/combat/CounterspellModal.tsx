...
export function CounterspellModal() {
  ...
  // Auto-dismiss
  const headCastId = head?.castId;
  useEffect(() => {
    if (!headCastId) return;
  ...
  const canCounter = eligibility?.canCounter ?? false;
  useEffect(() => {
    if (headCastId && hasEligibility && !canCounter) {
      queue.shift();
      notify();
    }
  }, [headCastId, hasEligibility, canCounter]);
    ...
    >
      ...
      >
        ...
        >
          ...
          <strong style={{ color: theme.purple }}>{head.spellLevel}</strong> spell. You can cast
          Counterspell at level {head.spellLevel} or higher to <strong>auto-cancel</strong>, or at a
          lower level to make a DC {10 + head.spellLevel} ability check.
        </div>
...
function checkEligibility(
  ...
  if (!myToken || myToken.id === head.casterTokenId) return empty;
  ...
  if (caster) {
    const dx = caster.x - myToken.x;
    const dy = caster.y - myToken.y;
