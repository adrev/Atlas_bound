...
export function LootEditor({ characterId, tokenName, onClose, canEdit = true }: LootEditorProps) {
  ...
  const resetCustomForm = () => {
    ...
    setCustomCost('');
    setCustomAC('');
    setCustomACType('flat');
    setCustomStealthDis(false);
    setCustomStrReq('');
    setCustomMagicBonus('0');
    setShowCustom(false);
  };
  ...
  const fetchPlayerCharacters = useCallback(() => {
    fetch('/api/characters')
      ...
      .then((chars: PlayerCharacter[]) => {
        ...
        setPlayerCharacters(chars.filter((c) => c.id !== characterId && c.race !== 'loot'));
      })
      .catch(() => {});
  }, [characterId]);
  ...
  const dropItem = async (entry: LootEntry) => {
    try {
      ...
      const creatureToken = Object.values(tokens).find((t) => t.characterId === characterId);
      const dropX = creatureToken ? creatureToken.x + 70 : currentMap.width / 2;
      const dropY = creatureToken ? creatureToken.y : currentMap.height / 2;
      ...
      await fetch(`/api/characters/${charData.id}/loot`, {
        ...
        body: JSON.stringify({
          ...
          itemSlug: entry.item_slug || null,
          itemRarity: entry.item_rarity || 'common',
          quantity: 1,
        }),
      });
      ...
      <div style={S.panel}>
        ...
        <div style={S.header}>
          <div style={S.headerIcon}>
            <img
              src={getCreatureImageUrl(tokenName || 'Loot')}
              alt=""
              style={{ width: 32, height: 32, borderRadius: '50%', objectFit: 'cover' }}
              onError={(e) => {
                (e.target as HTMLImageElement).src = getCreatureIconUrl(tokenName || 'Loot');
              }}
            />
          </div>
          <div style={{ flex: 1 }}>
            ...
            {searchResults.length > 0 && !showCustom && (
              <div style={S.searchDropdown}>
                {searchResults.map((r) => (
                  ...
                  >
                    <img
                      src={getItemImageUrl(r.slug || r.name)}
                      alt=""
                      loading="lazy"
                      style={S.searchItemImg}
                      onError={(e) => {
                        (e.target as HTMLImageElement).src = getItemIconUrl(r.name, r.type);
                      }}
                    />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div
                        style={{
                          ...
                          fontWeight: 600,
                          color:
                            RARITY_COLORS[r.rarity?.toLowerCase() || 'common'] ||
                            theme.text.primary,
                        }}
                      >
                        {r.name}
              ...
              >
                ...
                <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                  ...
                  <select
                    ...
                    style={{
                      ...S.raritySelect,
                      flex: 1,
                      color: RARITY_COLORS[customRarity] || theme.text.primary,
                    }}
                  >
                ...
                {customType === 'weapon' && (
                  <>
                    ...
                    >
                      Pick a base weapon, then modify the stats below. Finesse weapons use DEX
                      instead of STR for attacks.
                    </div>
                    <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                      <div style={{ flex: 2 }}>
                        <div style={S.fieldLabel}>Magic Bonus (+X to hit and damage)</div>
                        <select
                          value={customMagicBonus}
                          onChange={(e) => setCustomMagicBonus(e.target.value)}
                          style={S.raritySelect}
                        >
                    ...
                    <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                      {(customProperties.includes('Thrown') ||
                        customProperties.includes('Ammunition')) && (
                        <div style={{ flex: 1 }}>
                  ...
                  <>
                    <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                      <div style={{ flex: 1 }}>
                        <div style={S.fieldLabel}>
                          {customType === 'shield' ? 'AC Bonus' : 'Base AC'}
                        </div>
                        <input
                          type="number"
                          value={customAC}
                          onChange={(e) => setCustomAC(e.target.value)}
                          placeholder={customType === 'shield' ? '2' : '14'}
                          style={S.raritySelect}
                        />
                      </div>
                      {customType === 'armor' && (
                    ...
                    <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={customStealthDis}
                          onChange={(e) => setCustomStealthDis(e.target.checked)}
                        />
                      ...
                      {customType === 'armor' && (
                        <div style={{ flex: 1 }}>
                          ...
                          <input
                            type="number"
                            value={customStrReq}
                            onChange={(e) => setCustomStrReq(e.target.value)}
                            placeholder="0"
                            style={{ ...S.raritySelect, width: 40 }}
                          />
                        </div>
                      )}
                    </div>
          ...
          {loot.length === 0 ? (
            <div style={S.emptyState}>
              ...
              <div style={{ fontSize: 10, color: theme.text.muted, marginTop: 2 }}>
                Search items above or add custom loot
              </div>
            </div>
          ) : (
            loot.map((entry) => (
              <div key={entry.id} style={S.lootItem}>
                {canEdit ? (
                  <button
                    ...
                    style={{
                      ...
                      flexShrink: 0,
                      cursor: 'pointer',
                      border: entry.equipped
                        ? `2px solid ${theme.gold.primary}`
                        : `2px solid ${theme.border.default}`,
                      background: entry.equipped ? theme.gold.bg : 'transparent',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                      fontSize: 10,
                      color: entry.equipped ? theme.gold.primary : theme.text.muted,
                      padding: 0,
                ...
                <img
                  src={getItemImageUrl(entry.item_slug || entry.item_name)}
                  alt=""
                  loading="lazy"
                  style={S.lootItemImg}
                  onError={(e) => {
                    (e.target as HTMLImageElement).src = getItemIconUrl(entry.item_name);
                  }}
                />
                <div
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 8, color: theme.text.muted, textTransform: 'capitalize' }}
                  >
                    {entry.item_rarity}
                ...
                {canEdit ? (
                  ...
                  <span style={{ ...S.qtyValue, marginLeft: 4, marginRight: 4 }}>
                    x{entry.quantity}
                  </span>
                )}
                {canEdit && (
                  <button
                    onClick={() => dropItem(entry)}
                    title="Drop on map"
                    style={{
                      ...S.removeBtn,
                ...
                {canEdit && (
                  <div style={{ position: 'relative' }}>
                    <button
                      onClick={() =>
                        setTransferEntryId(transferEntryId === entry.id ? null : entry.id)
                      }
