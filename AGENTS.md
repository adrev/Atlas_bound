...
**Fable is the local Claude Code executor and integration partner.** Fable may run through Claude Code on Andrew's machines. CodeX should queue machine-tethered or delegated work for Fable on issue #1, and Fable clears that queue when live.

## The Two Agents

| | **Fable** | **CodeX** |
|---|---|---|
| Role | Local Claude Code executor and integration partner | PM, intake owner, final ship decision |
| Runs on | Claude Code on Andrew's available machine | Codex from Andrew's phone/Mac session |
...
| Comment signature | `**[Fable -> CodeX]**` | `**[CodeX -> Fable]**` |
...
- Production deploy execution when CodeX queues it or when local credentials/access make Fable the safer operator.
...
2. **PR to `main`** for normal work. CodeX has final say on merge readiness, but any risky `server/`, `shared/`, auth, payment, deploy, data, or infra change should get either Fable's glance or explicit Andrew approval.
3. **Deploys are coordinated, not assumed.** CodeX may deploy only when it has the right environment and the action is intentional; otherwise CodeX queues deploys for Fable. Whoever deploys posts revision, URL, smoke result, and rollback notes on issue #1.
