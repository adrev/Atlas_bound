...
| Save modifiers | `core-5e` | partial | P1 | Slow and cover flat save modifiers are applied in the shared rules, server `!save`, client roll engine, base subclass saves, legacy save-or-suck helpers, Tier 11 feature saves, Tier 13 Channel Divinity saves, Monk Stunning Strike, Turn Undead, misc class helpers such as Breath Weapon/Battle Master maneuvers, and hazard helpers such as disease/poison; race save traits are separate from damage resistance; overlapping labels such as magic plus charmed are supported; individual spell command helpers still need normalization. |
| Spell AoE commands | `manual-helper` -> `core-5e` | inconsistent | P1 | Tier 12, Tier 16, and Tier 21 saves now use shared save modifier math, but broader spell damage/HP/defense handling still needs server resolver normalization. |
