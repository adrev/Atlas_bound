...
| Save modifiers | `core-5e` | partial | P1 | Slow and cover flat save modifiers are applied in the shared rules, server `!save`, and client roll engine; race save traits are separate from damage resistance; overlapping labels such as magic plus charmed are supported; individual spell command helpers still need normalization. |
